...
class CarsControllers {
    ...
    getCarsList = async (req, res) => {
        try {
            ...
            res.render('cars/carList', { title: 'Список Автівок', cars, user: req.user || null })
        } catch (error) {
    ...
    getCarById = async (req, res) => {
        try {
            ...
            res.render('cars/carDetails', { title: car.title, car, user: req.user || null })
        } catch (error) {
            res.status(500).render('error', {
                error: {
                    status: 500,
    ...
    getCarForm = async (req, res) => {
        try {
            ...
            res.render('cars/carForm', { car, errors: [], user: req.user || null })
        } catch (error) {
            res.status(500).render('error', {
                error: {
                    status: 500,
